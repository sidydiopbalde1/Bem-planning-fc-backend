...
import { StatusModule } from '@prisma/client';
...
export class ModulesService {
  ...
  async findAll(pagination: PaginationDto, filters?: { programmeId?: string; intervenantId?: string; status?: string }) {
  ...
  async create(data: any, userId: string, userName?: string) {
    ...
    if (existing) {
      throw new ConflictException('Un module avec ce code existe déjà dans ce programme');
    }
    ...
    if (vht <= 0) {
      throw new BadRequestException('Le volume horaire total doit être supérieur à 0');
    }

    // Handle empty intervenantId
  ...
  async update(id: string, data: any, currentUserId?: string, currentUserName?: string) {
    ...
    // Handle intervenantId - empty string means remove the intervenant
  ...
  async remove(id: string, currentUserId?: string, currentUserName?: string) {
    ...
    if (seancesFutures > 0) {
      throw new BadRequestException('Impossible de supprimer: ce module a des séances futures');
    }
  ...
  async updateProgression(moduleId: string) {
    const module = await this.prisma.module.findUnique({
      where: { id: moduleId },
    });
